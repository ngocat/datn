...
void aht20_init_b3(unsigned char addr)
{
    ...
    i2c3_start();
    i2c3_write_byte(addr << 1);
    i2c3_write_byte(0xBE);
    i2c3_write_byte(0x08);
    i2c3_write_byte(0x00);
    i2c3_stop();
...
int aht20_read_b3(unsigned char addr, int *temp_x10, int *hum_x10)
{
    ...
    i2c3_start();
    i2c3_write_byte(addr << 1);
    i2c3_write_byte(0xAC);
    i2c3_write_byte(0x33);
    i2c3_write_byte(0x00);
    i2c3_stop();
    ...
    i2c3_start();
    i2c3_write_byte((addr << 1) | 1);
    for (i = 0; i < 5; i++)
        data[i] = i2c3_read_byte(1);
    data[5] = i2c3_read_byte(0);
    i2c3_stop();
